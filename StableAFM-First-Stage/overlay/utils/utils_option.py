    ...
    for phase, dataset in opt['datasets'].items():
        ...
        for key in ('dataroot_H', 'dataroot_L'):
            ...
            if isinstance(value, list):
                dataset[key] = [os.path.expanduser(path) for path in value]
            elif value is not None:
                dataset[key] = os.path.expanduser(value)
        if dataset.get('image_manifest') is not None:
            dataset['image_manifest'] = os.path.expanduser(dataset['image_manifest'])
...
def find_last_checkpoint(save_dir, net_type='G', pretrained_path=None):
    ...
    pattern = re.compile(r'^(\d+)_{}\.pth$'.format(re.escape(net_type)))
    checkpoints = []
    for file_ in file_list:
        match = pattern.match(os.path.basename(file_))
        if match:
            checkpoints.append((int(match.group(1)), file_))

    if checkpoints:
        init_iter, init_path = max(checkpoints, key=lambda item: item[0])
    else:
        init_iter = 0
