...
def calculate_psnr(img1, img2, border=0, data_range=255.0):
    # img1 and img2 have range [0, data_range]
    #img1 = img1.squeeze()
    #img2 = img2.squeeze()
    ...
    if mse == 0:
        return float('inf')
    return 20 * math.log10(float(data_range) / math.sqrt(mse))


# --------------------------------------------
# SSIM
# --------------------------------------------
def calculate_ssim(img1, img2, border=0, data_range=255.0):
    '''calculate SSIM
    ...
    if img1.ndim == 2:
        ...
        if img1.shape[2] == 3:
            ssims = []
            for i in range(3):
                ssims.append(ssim(img1[:,:,i], img2[:,:,i], data_range=data_range))
            return np.array(ssims).mean()
        elif img1.shape[2] == 1:
            return ssim(np.squeeze(img1), np.squeeze(img2), data_range=data_range)
    else:
