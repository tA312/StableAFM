...
class ModelPlain(ModelBase):
    ...
    def test(self):
        self.netG.eval()
        with torch.no_grad():
            # Rank-zero-only validation must bypass DDP's forward-time buffer sync.
            if self.opt_train['test_use_ema'] and hasattr(self, 'netE'):
                test_net = self.netE
            else:
                test_net = self.get_bare_model(self.netG)
            self.E = test_net(self.L)
        self.netG.train()
