...
class DataModuleFromConfig(pl.LightningDataModule):
    def __init__(self, batch_size, train=None, validation=None, test=None, predict=None,
                 ...
                 shuffle_val_dataloader=False, pin_memory=False, persistent_workers=False,
    ...
    def _common_loader_kwargs(self):
        kwargs = {
    ...
    def _train_dataloader(self):
        ...
        return DataLoader(self.datasets["train"], batch_size=self.batch_size,
                          ...
                          worker_init_fn=init_fn, drop_last=self.drop_last,
...
class BestCheckpointLinkCallback(Callback):
    """Expose Lightning's monitored best checkpoint as ``best.ckpt``."""

    def __init__(self, filename='best.ckpt', required=True):
        super().__init__()
    ...
    def on_train_end(self, trainer, pl_module):
        ...
        if not best_path:
            if self.required:
                raise RuntimeError('Training ended without a monitored best checkpoint.')
            return
        checkpoint_dir = os.path.dirname(best_path)
        destination = os.path.join(checkpoint_dir, self.filename)
        temporary = destination + '.tmp'
        if os.path.lexists(temporary):
            os.unlink(temporary)
        os.symlink(os.path.basename(best_path), temporary)
        os.replace(temporary, destination)
        print(f'Best checkpoint link: {destination} -> {os.path.basename(best_path)}')
