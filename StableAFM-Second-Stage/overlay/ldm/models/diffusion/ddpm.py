...
class LatentDiffusionSRTextWT(DDPM):
    ...
    def __init__(self,
                 ...
                 validation_seed=23,
                 validation_modalities=None,
                 paired_x0_latent_l1_weight=0.0,
                 paired_x0_pixel_l1_weight=0.0,
                 paired_x0_gradient_l1_weight=0.0,
                 paired_measurement_l2_weight=0.0,
                 paired_auxiliary_max_t=499,
                 paired_auxiliary_decode_batch_size=1,
                 paired_measurement_scale=4,
                 paired_measurement_phase_y=0,
                 paired_measurement_phase_x=0,
                 *args, **kwargs):
        ...
        self.deterministic_validation = bool(deterministic_validation)
        self.validation_seed = int(validation_seed)
        self.validation_modalities = tuple(validation_modalities or ())
        self.paired_x0_latent_l1_weight = float(paired_x0_latent_l1_weight)
        self.paired_x0_pixel_l1_weight = float(paired_x0_pixel_l1_weight)
        self.paired_x0_gradient_l1_weight = float(paired_x0_gradient_l1_weight)
        self.paired_measurement_l2_weight = float(paired_measurement_l2_weight)
        self.paired_auxiliary_max_t = int(paired_auxiliary_max_t)
        self.paired_auxiliary_decode_batch_size = int(
            paired_auxiliary_decode_batch_size
        )
        self.paired_measurement_scale = int(paired_measurement_scale)
        self.paired_measurement_phase_y = int(paired_measurement_phase_y)
        self.paired_measurement_phase_x = int(paired_measurement_phase_x)
        paired_loss_weights = {
            'paired_x0_latent_l1_weight': self.paired_x0_latent_l1_weight,
            'paired_x0_pixel_l1_weight': self.paired_x0_pixel_l1_weight,
            'paired_x0_gradient_l1_weight': self.paired_x0_gradient_l1_weight,
            'paired_measurement_l2_weight': self.paired_measurement_l2_weight,
        }
        if any(not np.isfinite(value) or value < 0.0
               for value in paired_loss_weights.values()):
            raise ValueError(
                'Paired auxiliary loss weights must be finite and '
                f'non-negative: {paired_loss_weights}'
            )
        if any(paired_loss_weights.values()) and not self.paired_input:
            raise ValueError(
                'Paired auxiliary reconstruction losses require '
                'paired_input=True.'
            )
        if not 0 <= self.paired_auxiliary_max_t < int(kwargs['timesteps']):
            raise ValueError(
                'paired_auxiliary_max_t must be within the diffusion schedule.'
            )
        if self.paired_auxiliary_decode_batch_size <= 0:
            raise ValueError(
                'paired_auxiliary_decode_batch_size must be positive.'
            )
        if self.paired_measurement_scale <= 0:
            raise ValueError('paired_measurement_scale must be positive.')
        if not 0 <= self.paired_measurement_phase_y < self.paired_measurement_scale:
            raise ValueError(
                'paired_measurement_phase_y is outside the sampling phase.'
            )
        if not 0 <= self.paired_measurement_phase_x < self.paired_measurement_scale:
            raise ValueError(
                'paired_measurement_phase_x is outside the sampling phase.'
            )
        if self.deterministic_validation and not self.paired_input:
            raise ValueError('deterministic_validation requires paired_input=True.')
        if len(set(self.validation_modalities)) != len(self.validation_modalities):
            raise ValueError('validation_modalities must contain unique names.')
        assert self.num_timesteps_cond <= kwargs['timesteps']
        # for backwards compatibility after implementation of DiffusionWrapper
        if conditioning_key is None:
            conditioning_key = 'concat' if concat_mode else 'crossattn'
        ...
        if self.paired_input:
            if self.unfrozen_diff:
                raise ValueError('paired_input requires unfrozen_diff=False.')
            # Paired SwinIR training is deliberately limited to the
            # time-aware encoder and SFT/SPADE layers.
            for param in self.first_stage_model.parameters():
                param.requires_grad = False
            for param in self.cond_stage_model.parameters():
                param.requires_grad = False
            for name, param in self.model.named_parameters():
                param.requires_grad = 'spade' in name
            for param in self.structcond_stage_model.parameters():
                param.requires_grad = True
    ...
    def _get_paired_input(self, batch, return_first_stage_outputs=False,
                          return_original_cond=False, bs=None, deterministic=False):
        """Encode same-size grayscale SwinIR/HR pairs without degradation."""
        if 'lq' not in batch or 'gt' not in batch:
            raise KeyError('paired_input batches must contain both "lq" and "gt".')

        im_lq = batch['lq'].to(self.device, memory_format=torch.contiguous_format).float()
        im_gt = batch['gt'].to(self.device, memory_format=torch.contiguous_format).float()
        if im_lq.ndim != 4 or im_gt.ndim != 4:
            raise ValueError(f'Expected BCHW tensors, got LQ {im_lq.shape}, GT {im_gt.shape}.')
        if im_lq.shape != im_gt.shape:
            raise ValueError(f'Paired LQ/GT shapes differ: {im_lq.shape} vs {im_gt.shape}.')
        if im_lq.shape[1] != 1:
            raise ValueError(f'Expected single-channel grayscale input, got {im_lq.shape[1]} channels.')

        if bs is not None:
            im_lq = im_lq[:bs]
            im_gt = im_gt[:bs]
        # The frozen SD 2.1 VAE remains RGB. Replication is exact and is the
        # only channel conversion performed in the training input path.
        self.lq = im_lq.repeat(1, 3, 1, 1).mul(2.0).sub(1.0).clamp(-1.0, 1.0)
        self.gt = im_gt.repeat(1, 3, 1, 1).mul(2.0).sub(1.0).clamp(-1.0, 1.0)

        encoder_posterior = self.encode_first_stage(self.lq)
        if deterministic and isinstance(encoder_posterior, DiagonalGaussianDistribution):
            z = (self.scale_factor * encoder_posterior.mode()).detach()
        else:
            z = self.get_first_stage_encoding(encoder_posterior).detach()
        encoder_posterior_gt = self.encode_first_stage(self.gt)
        if deterministic and isinstance(encoder_posterior_gt, DiagonalGaussianDistribution):
            z_gt = (self.scale_factor * encoder_posterior_gt.mode()).detach()
        else:
    ...
    def get_input(self, batch, k=None, return_first_stage_outputs=False, force_c_encode=False,
                  cond_key=None, return_original_cond=False, bs=None, val=False, text_cond=None, return_gt=False, resize_lq=True):
    ...
    @staticmethod
    def _slice_paired_batch(batch, indices):
        """Select batch rows while preserving string metadata from collation."""
        batch_size = len(batch['sample_id'])
        index_tensor = torch.as_tensor(indices, dtype=torch.long)
        selected = {}
        for key, value in batch.items():
            if isinstance(value, torch.Tensor) and value.ndim and value.shape[0] == batch_size:
                selected[key] = value.index_select(0, index_tensor.to(value.device))
    ...
    def _paired_validation_randomness(self, sample_ids, reference):
        if len(sample_ids) != reference.shape[0]:
            raise ValueError('sample_id count does not match the paired validation batch.')
        timesteps = []
        noises = []
        for sample_id in sample_ids:
            digest = hashlib.sha256(
                f'{self.validation_seed}:{sample_id}'.encode('utf-8')
            ).digest()
            seed = int.from_bytes(digest[:8], byteorder='little') % (2 ** 63 - 1)
            generator = torch.Generator(device='cpu')
            generator.manual_seed(seed)
            timesteps.append(torch.randint(
                0, self.num_timesteps, (1,), generator=generator, dtype=torch.long
            ))
            noises.append(torch.randn(
                tuple(reference.shape[1:]), generator=generator, dtype=torch.float32
            ))
        timestep_tensor = torch.cat(timesteps).to(reference.device)
        noise_tensor = torch.stack(noises).to(reference.device, dtype=reference.dtype)
        return timestep_tensor, noise_tensor

    def shared_step(self, batch, deterministic_validation=False, **kwargs):
        x, c, gt = self.get_input(
            batch, self.first_stage_key, val=deterministic_validation
        )
        timestep = None
        noise = None
        if deterministic_validation:
            if 'sample_id' not in batch:
                raise KeyError('Deterministic validation requires sample_id metadata.')
            timestep, noise = self._paired_validation_randomness(batch['sample_id'], gt)
        loss = self(x, c, gt, t=timestep, noise=noise)
        return loss

    def forward(self, x, c, gt, t=None, noise=None, *args, **kwargs):
        if t is None:
            ...
            t = t.to(self.device).long()
            if t.shape != (x.size(0),):
                raise ValueError(f'Expected timestep shape {(x.size(0),)}, got {t.shape}.')
            index = t.detach().cpu().numpy()
        ...
        return self.p_losses(gt, c, struc_c, t, t_ori, x, noise=noise, *args, **kwargs)

    def on_validation_epoch_start(self):
        if not (self.paired_input and self.deterministic_validation):
            return
        scopes = ('all',) + self.validation_modalities
        metrics = ('loss_simple', 'loss_vlb', 'loss')
        self._paired_validation_stats = {
            scope: {
                metric: torch.zeros(2, dtype=torch.float64, device=self.device)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if 'modality' not in batch or 'sample_id' not in batch:
            raise KeyError(
                'Deterministic multimodal validation requires modality and sample_id.'
            )

        # DistributedSampler pads a validation set whose size is not divisible
        # by world size.  Keep each original dataset index only on its canonical
        # rank so a capped validation subset is evaluated exactly once, without
        # over-weighting the duplicated prefix.
        if torch.distributed.is_available() and torch.distributed.is_initialized():
            if 'dataset_index' not in batch:
                raise KeyError('Distributed deterministic validation requires dataset_index.')
            rank = torch.distributed.get_rank()
            world_size = torch.distributed.get_world_size()
            dataset_indices = batch['dataset_index']
            if torch.is_tensor(dataset_indices):
                dataset_indices = dataset_indices.detach().cpu().tolist()
            owned = [
                position for position, dataset_index in enumerate(dataset_indices)
                if int(dataset_index) % world_size == rank
            ]
            if not owned:
                return {}
            batch = self._slice_paired_batch(batch, owned)

        modalities = list(batch['modality'])
        unknown = sorted(set(modalities) - set(self.validation_modalities))
        if unknown:
            raise ValueError(f'Unexpected validation modalities: {unknown}.')
        for modality in self.validation_modalities:
            indices = [index for index, value in enumerate(modalities) if value == modality]
            if not indices:
                continue
            subbatch = self._slice_paired_batch(batch, indices)
            _, loss_dict = self.shared_step(
                subbatch, deterministic_validation=True
            )
            count = float(len(indices))
            for metric in ('loss_simple', 'loss_vlb', 'loss'):
                value = loss_dict[f'val/{metric}'].detach().double()
                self._paired_validation_stats['all'][metric][0].add_(value * count)
                self._paired_validation_stats['all'][metric][1].add_(count)
                self._paired_validation_stats[modality][metric][0].add_(value * count)
                self._paired_validation_stats[modality][metric][1].add_(count)
        return {}
    ...
    def validation_epoch_end(self, outputs):
        ...
        sanity_checking = bool(getattr(self.trainer, 'sanity_checking', False))
        for scope, metrics in self._paired_validation_stats.items():
            for metric, totals in metrics.items():
                if distributed:
                    torch.distributed.all_reduce(totals, op=torch.distributed.ReduceOp.SUM)
                if totals[1].item() <= 0:
                    # Lightning's optional sanity check only visits the first few
                    # validation batches.  With samples sorted by modality, that
                    # prefix is not required to contain every modality.  The real
                    # full validation epoch remains strict.
                    if sanity_checking:
                        continue
                    raise RuntimeError(f'No validation samples were seen for {scope}.')
                value = (totals[0] / totals[1]).float()
                name = f'val/{metric}' if scope == 'all' else f'val/{scope}_{metric}'
                self.log(
                    name, value, prog_bar=(name == 'val/loss_simple'),
    ...
    def _paired_auxiliary_loss_enabled(self):
        return any((
            self.paired_x0_latent_l1_weight,
            self.paired_x0_pixel_l1_weight,
            self.paired_x0_gradient_l1_weight,
            self.paired_measurement_l2_weight,
        ))

    def _paired_predict_x0(self, x_noisy, t, model_output):
        if self.parameterization == 'eps':
            return self.predict_start_from_noise(
                x_noisy, t=t, noise=model_output
            )
        if self.parameterization == 'x0':
            return model_output
        if self.parameterization == 'v':
            return self.predict_start_from_z_and_v(
                x_noisy, model_output, t
            )
        raise NotImplementedError(self.parameterization)

    @staticmethod
    def _paired_weighted_mean(per_image, weights, mask, zero):
        selected_mask = mask.to(weights.dtype)
        denominator = selected_mask.sum()
        if not bool(mask.any()):
            return zero
        return (
            per_image * weights * selected_mask
        ).sum() / denominator.clamp_min(1.0)

    def _paired_auxiliary_losses(self, predicted_x0, x_start, t):
        """Compute bounded low/mid-t reconstruction losses for paired SR.

        Alpha-bar weights suppress unstable clean-latent estimates near the
        pure-noise end of the schedule. Pixel-space terms decode at most the
        configured number of eligible samples per rank to bound memory use.
        """
        ...
        eligible = t <= self.paired_auxiliary_max_t
        if not bool(eligible.any()):
            return losses
        alpha_weights = self.alphas_cumprod.to(
            device=t.device, dtype=predicted_x0.dtype
        ).gather(0, t)
        latent_per_image = (
            predicted_x0 - x_start
        ).abs().flatten(1).mean(1)
        losses['x0_latent_l1'] = self._paired_weighted_mean(
            latent_per_image, alpha_weights, eligible, zero
        )
        ...
        if not hasattr(self, 'gt'):
            raise RuntimeError(
                'Paired pixel losses require the GT image cached by get_input.'
            )
        indices = torch.nonzero(
            eligible, as_tuple=False
        ).flatten()[:self.paired_auxiliary_decode_batch_size]
        predicted_pixels = self.differentiable_decode_first_stage(
            predicted_x0.index_select(0, indices)
        )
        if predicted_pixels.shape[1] == 3:
            predicted_gray = (
                predicted_pixels[:, 0:1] * 0.299
                + predicted_pixels[:, 1:2] * 0.587
                + predicted_pixels[:, 2:3] * 0.114
            )
        elif predicted_pixels.shape[1] == 1:
            predicted_gray = predicted_pixels
        else:
            raise ValueError(
                f'Unexpected decoded channel count: {predicted_pixels.shape[1]}'
            )
        predicted_gray = (predicted_gray + 1.0) * 0.5
        target_gray = (
            self.gt.index_select(0, indices)[:, 0:1] + 1.0
        ) * 0.5
        decode_weights = alpha_weights.index_select(0, indices)
        decode_mask = torch.ones_like(decode_weights, dtype=torch.bool)

        pixel_per_image = (
            predicted_gray - target_gray
        ).abs().flatten(1).mean(1)
        losses['x0_pixel_l1'] = self._paired_weighted_mean(
            pixel_per_image, decode_weights, decode_mask, zero
        )

        predicted_dx = predicted_gray[..., :, 1:] - predicted_gray[..., :, :-1]
        target_dx = target_gray[..., :, 1:] - target_gray[..., :, :-1]
        predicted_dy = predicted_gray[..., 1:, :] - predicted_gray[..., :-1, :]
        target_dy = target_gray[..., 1:, :] - target_gray[..., :-1, :]
        gradient_per_image = 0.5 * (
            (predicted_dx - target_dx).abs().flatten(1).mean(1)
            + (predicted_dy - target_dy).abs().flatten(1).mean(1)
        )
        losses['x0_gradient_l1'] = self._paired_weighted_mean(
            gradient_per_image, decode_weights, decode_mask, zero
        )

        phase_y = self.paired_measurement_phase_y
        phase_x = self.paired_measurement_phase_x
        scale = self.paired_measurement_scale
        predicted_measurement = predicted_gray[
            ..., phase_y::scale, phase_x::scale
        ]
        target_measurement = target_gray[
            ..., phase_y::scale, phase_x::scale
        ]
        measurement_per_image = (
            predicted_measurement - target_measurement
        ).square().flatten(1).mean(1)
        losses['measurement_l2'] = self._paired_weighted_mean(
            measurement_per_image, decode_weights, decode_mask, zero
        )
        return losses
    ...
    def p_losses(self, x_start, cond, struct_cond, t, t_ori, z_gt, noise=None):
        ...
        if self.paired_input and self._paired_auxiliary_loss_enabled():
            predicted_x0 = self._paired_predict_x0(
                x_noisy, t, model_output_
            )
            auxiliary_losses = self._paired_auxiliary_losses(
                predicted_x0, x_start, t
            )
            auxiliary_loss = (
                self.paired_x0_latent_l1_weight
                * auxiliary_losses['x0_latent_l1']
                + self.paired_x0_pixel_l1_weight
                * auxiliary_losses['x0_pixel_l1']
                + self.paired_x0_gradient_l1_weight
                * auxiliary_losses['x0_gradient_l1']
                + self.paired_measurement_l2_weight
                * auxiliary_losses['measurement_l2']
            )
            for name, value in auxiliary_losses.items():
                loss_dict[f'{prefix}/loss_{name}'] = value
            loss_dict[f'{prefix}/loss_auxiliary'] = auxiliary_loss
    ...
    def configure_optimizers(self):
        ...
        if self.paired_input:
            ...
            for name, param in self.named_parameters():
                allowed = (
                    name.startswith('structcond_stage_model.')
                    or (name.startswith('model.diffusion_model.') and '.spade.' in name)
                )
                if param.requires_grad and allowed:
            ...
            if unexpected_names:
                raise RuntimeError(
                    'Unexpected trainable parameters in paired-input mode: '
                    + ', '.join(unexpected_names[:20])
                )
            if not params:
                raise RuntimeError('No trainable Time-aware Encoder/SFT parameters were found.')
            print(
                f'Optimizing {len(allowed_names)} paired-input tensors '
                f'({sum(param.numel() for param in params):,} parameters).'
            )
        else:
            params = list(self.model.parameters())
            params = params + list(self.cond_stage_model.parameters())
        ...
        if self.learn_logvar:
            ...
            print('Diffusion model optimizing logvar')
            params.append(self.logvar)
        optimizer_cls = torch.optim.Adam if self.paired_input else torch.optim.AdamW
        opt = optimizer_cls(params, lr=lr, weight_decay=0.0)
        if self.use_scheduler:
            assert 'target' in self.scheduler_config
            scheduler = instantiate_from_config(self.scheduler_config)
